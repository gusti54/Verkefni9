...
const program = (() => {
  ...
  function displayCompany(companiesList) {
    if (companiesList.length === 0) {
      displayError('Ekkert fyrirtæki fannst');
      return;
    }
    const container = companies.querySelector('.results');
    removeAll(container);
  ...
  function displayLoading() {
    ...
    const loadingText = document.createTextNode('Leitar að fyrirtækjum');
  ...
  function onSubmit(e) {
    ...
    if (input.value.trim().length > 0) {
      ...
      displayError('Ekkert fyrirtæki fannst. Þú verður að skrifa fyrirtækis nafn í dálkinn.');
    }
  }
